...
public class EmbeddedDerby {
	...
	// Try to avoid conflicting with other files since databases are created in the
	// current
	// directory. This also makes it easier to clean up if something goes wrong
	private static final String NAME_PREFIX = "/kafka_test"; // "__test_database_";
	private static final String PROTOCOL = "jdbc:mysql://192.168.52.130:3306";// "jdbc:derby:";

	private String name;
	private Connection conn;

	private String user = "roottest";
...
//    this("default");
	...
	public EmbeddedDerby(String name) {
		...
		// Derby seems to have problems with shutdown + restart with new connection in a
		// process. We
		// have to manually make sure it's initialized by instantiating a driver
		// instance. This only
		// seems to be necessary in some cases (between test suites, but not between
		// test cases for
...
//    return getUrl(true);
	 ...
	 * @param fields list of field names followed by specs specifications, e.g.
	 *               "user-id", "INT NOT NULL", "username", "VARCHAR(20)". May
	 *               include other settings like "PRIMARY KEY user_id"
	 */
	...
	public void close() throws SQLException {
		...
		// Derby requires more than just closing the connection to clear out the
		// embedded data
		try {
	...
	/**
	 * Drops the database by deleting it's files from disk. This assumes the working
	 * directory isn't changing so the database files can be found relative to the
	 ...
	 * @param where the condition rows must match; be careful to correctly
	 *              quote/escape any strings, table names, or literal
	 * @throws SQLException
	 */
