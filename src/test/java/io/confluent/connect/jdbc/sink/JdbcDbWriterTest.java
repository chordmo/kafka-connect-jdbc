...
public class JdbcDbWriterTest {
  ...
  public void autoCreateWithAutoEvolve() throws SQLException {
    ...
    Schema valueSchema1 = SchemaBuilder.struct().field("author", Schema.STRING_SCHEMA)
        .field("title", Schema.STRING_SCHEMA).build();

    Struct valueStruct1 =
        new Struct(valueSchema1).put("author", "Tom Robbins").put("title", "Villa Incognito");

    writer.write(Collections
        .singleton(new SinkRecord(topic, 0, keySchema, 1L, valueSchema1, valueStruct1, 0)));

    TableDefinition metadata =
        dialect.describeTable(writer.cachedConnectionProvider.getConnection(), tableId);
    ...
    Schema valueSchema2 = SchemaBuilder.struct().field("author", Schema.STRING_SCHEMA)
        .field("title", Schema.STRING_SCHEMA).field("year", Schema.OPTIONAL_INT32_SCHEMA) // new
                                                                                          // field
        .field("review", SchemaBuilder.string().defaultValue("").build()); // new field

    Struct valueStruct2 = new Struct(valueSchema2).put("author", "Tom Robbins")
        .put("title", "Fierce Invalids").put("year", 2016);

    writer.write(Collections
        .singleton(new SinkRecord(topic, 0, keySchema, 2L, valueSchema2, valueStruct2, 0)));
  ...
  public void multiInsertWithRecordValuePkFailsDueToUniqueConstraint() throws SQLException {
    writeSameRecordTwiceExpectingSingleUpdate(JdbcSinkConfig.InsertMode.INSERT,
        JdbcSinkConfig.PrimaryKeyMode.RECORD_VALUE, "author,title");
  }
  ...
  private void writeSameRecordTwiceExpectingSingleUpdate(JdbcSinkConfig.InsertMode insertMode,
      JdbcSinkConfig.PrimaryKeyMode pkMode, String pkFields) throws SQLException {
    String topic = "books";
    int partition = 7;
    long offset = 42;
    ...
    Schema keySchema = SchemaBuilder.struct().field("id", SchemaBuilder.INT64_SCHEMA);

    Struct keyStruct = new Struct(keySchema).put("id", 0L);

    Schema valueSchema = SchemaBuilder.struct().field("author", Schema.STRING_SCHEMA)
        .field("title", Schema.STRING_SCHEMA).build();

    Struct valueStruct =
        new Struct(valueSchema).put("author", "Tom Robbins").put("title", "Villa Incognito");
  ...
  public void sameRecordNTimes() throws SQLException {
    ...
    String createTable = "CREATE TABLE " + testId + " (" + "    the_byte  INTEGER,"
        + "    the_short INTEGER," + "    the_int INTEGER," + "    the_long INTEGER,"
        + "    the_float REAL," + "    the_double REAL," + "    the_bool  INTEGER,"
        + "    the_string TEXT," + "    the_bytes BLOB, " + "    the_decimal  NUMERIC,"
        + "    the_date  NUMERIC," + "    the_time  NUMERIC," + "    the_timestamp  NUMERIC" + ");";

    sqliteHelper.deleteTable(testId);
    sqliteHelper.createTable(createTable);

    Schema schema = SchemaBuilder.struct().name(testId).field("the_byte", Schema.INT8_SCHEMA)
        .field("the_short", Schema.INT16_SCHEMA).field("the_int", Schema.INT32_SCHEMA)
        .field("the_long", Schema.INT64_SCHEMA).field("the_float", Schema.FLOAT32_SCHEMA)
        .field("the_double", Schema.FLOAT64_SCHEMA).field("the_bool", Schema.BOOLEAN_SCHEMA)
        .field("the_string", Schema.STRING_SCHEMA).field("the_bytes", Schema.BYTES_SCHEMA)
        .field("the_decimal", Decimal.schema(2).schema()).field("the_date", Date.SCHEMA)
        .field("the_time", Time.SCHEMA).field("the_timestamp", Timestamp.SCHEMA);

    final java.util.Date instant = new java.util.Date(1474661402123L);

    final Struct struct =
        new Struct(schema).put("the_byte", (byte) -32).put("the_short", (short) 1234)
            .put("the_int", 42).put("the_long", 12425436L).put("the_float", 2356.3f)
            .put("the_double", -2436546.56457d).put("the_bool", true).put("the_string", "foo")
            .put("the_bytes", new byte[] {-32, 124}).put("the_decimal", new BigDecimal("1234.567"))
            .put("the_date", instant).put("the_time", instant).put("the_timestamp", instant);
    ...
    writer.write(
        Collections.nCopies(numRecords, new SinkRecord("topic", 0, null, null, schema, struct, 0)));

    assertEquals(numRecords,
        sqliteHelper.select("SELECT * FROM " + testId, new SqliteHelper.ResultSetReadCallback() {
          @Override
          public void read(ResultSet rs) throws SQLException {
            ...
            assertEquals(new java.sql.Date(((java.util.Date) struct.get("the_date")).getTime()),
                rs.getDate("the_date"));
            assertEquals(new java.sql.Time(((java.util.Date) struct.get("the_time")).getTime()),
                rs.getTime("the_time"));
            assertEquals(
