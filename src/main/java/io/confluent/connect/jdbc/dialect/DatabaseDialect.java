 ...
 * the correct type so that the converted values can be added to the {@link Struct}. The Dialect can
 * do this once for each result set, rather than having to do the logic for every row.</li>
 *
...
public interface DatabaseDialect extends ConnectionProvider {
     ...
     * Hive
     */
    Map<ColumnId, ColumnDefinition> describeColumns(ResultSetMetaData metadata, TableId tableId)
            throws SQLException;
    ...
    /**
     * Create the definition for the columns in the result set returned when querying the table.
     * This may not work if the table is empty.
     *
     * @param connection the database connection; may not be null
     ...
     *                      but may be empty
     * @param nonKeyColumns the identifiers of the other columns in the table; may not be null but
     *                      may be empty
    ...
    /**
     * Build the UPSERT or MERGE prepared statement expression to either insert a new record into
     * the given table or update an existing record in that table Variables for each key column
     * should also appear in the WHERE clause of the statement.
     *
     * @param table         the identifier of the table; may not be null
     * @param keyColumns    the identifiers of the columns in the primary/unique key; may not be null
     *                      but may be empty
    ...
    /**
     * Create a function that converts column values for the column defined by the specified
     * mapping.
     *
    ...
    interface ColumnConverter {
        ...
        /**
         * Get the column's value from the row at the current position in the result set, and
         * convert it to a value that should be included in the corresponding {@link Field} in the
         * {@link Struct} for the row.
         *
